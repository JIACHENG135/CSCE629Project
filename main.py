...
class Solution(object):
    ...
    def gene_graph_dense(self):
        ...
        return self.g
    def save(self):
        with open('graph_15000.pickle', 'wb') as f:  # Python 3: open(..., 'wb')
            pickle.dump([self.graph, self.weight, self.g,self.size], f)
        f.close()
...
if __name__ == '__main__':
    ...
    #         edges2 = G2.edges()
    #         G3 = nx.Graph()
    #         G3.add_node(self.sall[r])
    #         G3.add_node(self.tall[r])
    ...
    #         plt.savefig('mbp_total' + str(__counter__) +'#' + str(r) + '.png')
    #         # plt.show()
    #         # plt.close()
    #         # nx.draw(G1,pos, node_color = 'g',node_size = 5,edges=edges, edge_color=weights, width=weights,edge_cmap=plt.cm.Blues)
    #         nx.draw(G2,pos, node_color = 'r',node_size = 5,edges=edges2, edge_color='r', width=3.0)
    #         # nx.draw(G3,pos, node_color = 'r',node_size = 10)
    #         plt.savefig('mbp_path' + str(__counter__) +'#' + str(r) + '.png')
    #         plt.clf()
    #         # plt.show()
    #         # plt.close()
